...
func TestGetReturnsTaskResponse(t *testing.T) {
	...
	resp, err := uc.Get(context.Background(), stored.ID)
	if err != nil { t.Fatalf("get failed: %v", err) }
	if resp.ID != stored.ID.String() || resp.Title != "Backend" || resp.Priority != "HIGH" { t.Fatalf("unexpected response: %+v", resp) }
	if resp.AssigneeID == nil || *resp.AssigneeID != assigneeID.String() { t.Fatalf("unexpected assignee: %+v", resp.AssigneeID) }
	if resp.DueDate == "" { t.Fatal("expected due date to be formatted") }
}
...
func TestGetHidesSoftDeletedTask(t *testing.T) {
	...
	_, err = uc.Get(context.Background(), stored.ID)
	if err == nil || !strings.Contains(err.Error(), "NOT_FOUND") { t.Fatalf("expected not found for deleted task, got %v", err) }
}
