...
func (s taskRepoStub) SoftDelete(ctx context.Context, id uuid.UUID) error { if s.softDelete != nil { return s.softDelete(ctx,id) }; return nil }
...
func TestDeleteSoftDeletesExistingTask(t *testing.T) {
	...
	uc := NewUsecase(taskRepoStub{
		...
		softDelete: func(ctx context.Context, id uuid.UUID) error {
			called = true
			if id != stored.ID { t.Fatalf("unexpected id: %s", id) }
			return nil
		},
	})

	if err := uc.Delete(context.Background(), stored.ID); err != nil { t.Fatalf("delete failed: %v", err) }
	if !called { t.Fatal("expected SoftDelete to be called") }
}
...
func TestDeleteRejectsAlreadyDeletedTask(t *testing.T) {
	...
	uc := NewUsecase(taskRepoStub{
		getByID: func(ctx context.Context, id uuid.UUID) (*task.Task, error) { return stored, nil },
		softDelete: func(ctx context.Context, id uuid.UUID) error { t.Fatal("should not soft delete twice"); return nil },
	})

	err = uc.Delete(context.Background(), stored.ID)
	if err == nil || !strings.Contains(err.Error(), "NOT_FOUND") { t.Fatalf("expected not found, got %v", err) }
}
