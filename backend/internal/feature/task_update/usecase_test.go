...
func (s taskRepoStub) Update(ctx context.Context, task *task.Task) error {
	if s.update != nil {
...
func TestUpdateModifiesAllowedFieldsAndPersists(t *testing.T) {
	creatorID := uuid.New()
	stored, err := task.NewTask("Backend", "Old", task.PriorityMedium, nil, creatorID)
	if err != nil {
		t.Fatal(err)
	}
	var saved *task.Task
	...
	resp, err := uc.Update(context.Background(), creatorID, stored.ID, &UpdateTaskRequest{
		Title: ptr("Backend done"), Description: ptr("New"),
		Status: ptr("IN_PROGRESS"), Priority: ptr("HIGH"), DueDate: &due,
	})
	if err != nil {
		t.Fatalf("update failed: %v", err)
	}
	if saved == nil {
		t.Fatal("expected updated task to be persisted")
	}
	if saved.Title != "Backend done" || saved.Description != "New" || saved.Status != task.StatusInProgress || saved.Priority != task.PriorityHigh {
		t.Fatalf("unexpected saved task: %+v", saved)
	}
	if resp.Title != "Backend done" || resp.Status != "IN_PROGRESS" || resp.Priority != "HIGH" {
...
func TestUpdateRejectsInvalidStatusTransition(t *testing.T) {
	...
	_, err = uc.Update(context.Background(), creatorID, stored.ID, &UpdateTaskRequest{Status: ptr("DONE")})
	if err == nil || !strings.Contains(err.Error(), "invalid status transition") {
		t.Fatalf("expected invalid transition, got %v", err)
	}
}

func TestUpdateRejectsOtherUsersTask(t *testing.T) {
	creatorID := uuid.New()
	...
	_, err = uc.Update(context.Background(), otherUserID, stored.ID, &UpdateTaskRequest{Status: ptr("IN_PROGRESS")})
